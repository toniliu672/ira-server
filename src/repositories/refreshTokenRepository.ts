...
export class RefreshTokenRepository {
  ...
  async findByUserId(userId: string) {
    try {
      ...
      throw new ApiError(
        "FETCH_FAILED",
        "Gagal mengambil data refresh token",
        500
      );
    }
  }
  ...
  async findByToken(token: string) {
    try {
      ...
      const error = e as Error;
      console.error("Repository Find Token Error:", error.message);
      throw new ApiError("FETCH_FAILED", "Gagal mengambil data token", 500);
    }
  }
  ...
  async revokeToken(id: string) {
    try {
      ...
      const error = e as Error;
      console.error("Repository Revoke Token Error:", error.message);
      throw new ApiError("UPDATE_FAILED", "Gagal merevoke token", 500);
    }
  }
}
